  ...
  try {
    ...
    for (const item of generatedData) {
      ...
      const newQuestion = {
        ...
        // Add other options if the AI generates more, or adapt as needed
        // For now, we assume only A, B, C are generated by the prompt.
        correct_option: item.correct_answer_letter.toUpperCase(), // Ensure it's 'A', 'B', or 'C'
        explanation: item.explanation,
        explanation_image: '', // Default, no image from AI for now
        is_active: false,
        votingClosed: false,
        timer: null, // Or get default from config
        created_at: new Date()
      };

      // Use the existing database connection 'db' from server/index.js
      ...
      if (insertResult.insertedId) {
        ...
        // Optionally emit socket event for each new question, or a single event after all are saved
        // io.emit('question_created', { ...newQuestion, _id: insertResult.insertedId });
      }
    }
    ...
    res.status(201).json({
      ...
      // data: savedQuestions // Optionally return the saved questions
    });

  } catch (error) {
    console.error('Error in /api/ai/generate-questions:', error);
    // Specific error for API key issues if detectable
